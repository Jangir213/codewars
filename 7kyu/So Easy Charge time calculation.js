...
function calculateTime(battery, charger) {
  const stepOne = (battery * 0.85) / charger;
  const stepTwo = (battery * 0.1) / (charger * 0.5);
  const stepThree = (battery * 0.05) / (charger * 0.2);

  return Math.round((stepOne + stepTwo + stepThree) * 100) / 100;
}